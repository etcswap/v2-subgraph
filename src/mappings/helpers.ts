...
export const FACTORY_ADDRESS = '0x0307cd3d7da98a29e6ed0d2137be386ec1e4bc9c'
...
export function convertEthToDecimal(etc: BigInt): BigDecimal {
  return etc.toBigDecimal().div(exponentToBigDecimal(18))
}
...
export function createLiquiditySnapshot(position: LiquidityPosition, event: EthereumEvent): void {
  ...
  snapshot.token0PriceUSD = token0.derivedETC.times(bundle.etcPrice)
  snapshot.token1PriceUSD = token1.derivedETC.times(bundle.etcPrice)
